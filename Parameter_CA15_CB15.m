...
snapshots_py = py.BEASY_IN_OUT2.snapshots_for_given_parameters_and_IDs(py.list(parameters), parameters_np_array1, py.list(IDs), py.list(data_type), simulation_seed_folder, collection_dir, py.list(IDs_types));
...
if ~isfile(fullfile(calib_dir, calib_data_file_err_inc))
    ...
    all_position_dict_with_error = introduce_error_and_write_file({MPs_IDs, IDs_current_density.'},   all_position_data, calib_dir, calib_data_file_err_inc,3);
end
...
[plot_data, min_out_pos1] = plot_objective_with_surrogates(ax, DOE_range1, surrogates, out_frm_time_surr2, 'nmsq', data_type, [length(IDs{1}), length(IDs{2})],[0.7,0.3], [0.005,0.001]);
